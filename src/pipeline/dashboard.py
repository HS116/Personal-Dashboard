...
app.layout = html.Div([
    ...
    html.Div([
        html.H1("Country Comparison Dashboard", style={'text-align': 'center'})
    ]),
...
def create_country_dropdowns(info_type : str) -> html.Div:
    ...
    return html.Div([
            ...
            html.Div([
            dcc.Dropdown(["Germany", "Great Britain", "United States", "India"], id=f"select-country1-{info_type.lower()}", value="Germany", style={"width":"200px"}), 
            dcc.Dropdown(["Germany", "Great Britain", "United States", "India"], id=f"select-country2-{info_type.lower()}", value="United States", style={"width":"200px"})
            ], style={"display" : "flex", "margin" : "auto", "width" : "1800px", "justify-content" : "space-around"}),

            html.Div([
            dbc.Card("Something", id=f"country1-{info_type.lower()}", color="secondary", inverse=True, style={'height': '100vh', 'padding':'20px', 'flex': '1', 'margin-right': '10px', 'overflow':'auto'}), 
            dbc.Card("Something", id=f"country2-{info_type.lower()}", color="secondary", inverse=True, style={'height': '100vh', 'padding':'20px', 'flex': '1', 'margin-left': '10px', 'overflow':'auto'})
...
@app.callback(Output("country1-stocks", "children"),
              Input("select-country1-stocks", "value"))
...
@app.callback(Output("country2-stocks", "children"),
              Input("select-country2-stocks", "value"))
